...
"""
================================================================================
    [01] Price Cross SMA 20 RR2.5
    NAS100 M5 — Michele Piazzoli — Quant Strategy
================================================================================

Standalone backtest script for the #1 ranked strategy.

Strategy:
    - Trigger:      Price Cross SMA 20 (close crosses above/below SMA 20)
...
OUTPUT_BASE     = Path("/Users/capitalpro/Desktop/Strategia 1/[01] Price Cross SMA 20 RR2.5")
...
# Strategy parameters
TRIGGER_PERIOD  = 20       # SMA period for price cross
...
STRATEGY_NAME   = "Price Cross SMA 20 RR2.5"
ASSET           = "NAS100"
TIMEFRAME       = "M5"
...
# =============================================================================
#  SIGNAL GENERATION — Price Cross SMA
# =============================================================================

def generate_signals(df, period):
    df = df.copy()
    sma = calc_sma(df['close'], period)
    df['sig_long']  = (df['close'] > sma) & (df['close'].shift(1) <= sma.shift(1))
    df['sig_short'] = (df['close'] < sma) & (df['close'].shift(1) >= sma.shift(1))
...
def run_backtest(df):
    df = df.copy()
    df = generate_signals(df, TRIGGER_PERIOD)
...
──────────────────────────────────────────────────────────────────────────────
  Asset:              {ASSET}
  Timeframe:          {TIMEFRAME}
  Trigger:            Price Cross SMA {TRIGGER_PERIOD}
...
def main():
    ...
    # Copy this script to [04] Python code
    import shutil
